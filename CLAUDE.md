...
- **`playbooks/k8s.yml`** is the Kubernetes workstation toolkit: `kubectl` role (kubectl + helm + krew + the `kubectl-netshoot` krew plugin, cross-platform incl. a direct-download fallback) + `k9s` role (Homebrew on macOS, pacman on Arch, GitHub release tarball into `~/.local/bin` on Debian/Ubuntu and other Linux since there's no apt/generic package) + `kubeseal` role (Bitnami Sealed Secrets CLI; Homebrew on macOS, pinned GitHub release tarball into `~/.local/bin` on Linux since upstream has no `latest/download` alias).
...
- **`roles/raycast`** installs Raycast (Homebrew cask, macOS only — no Linux build exists) and is install-only: Raycast's settings live in its own internal database synced via a Raycast account, so there's no config file to symlink from this repo.
- **`roles/wifi_loc_control`** (macOS only, no-op elsewhere) installs [WiFiLocControl](https://github.com/vborodulin/wifi-loc-control) — vendored as the `wifi-loc-control/upstream` submodule — which switches the active macOS network location to match the current Wi-Fi SSID via a `~/Library/LaunchAgents` LaunchAgent. The role symlinks `wifi-loc-control/upstream/wifi-loc-control.sh` → `~/.local/bin` and `wifi-loc-control/alias.conf` → `~/.wifi-loc-control/alias.conf` (maps SSIDs to network-location names; currently `O2-Internet-5G-709` and `O2-Internet-709` both → `Home`), creates the `Home` network location if missing, and (re)loads the LaunchAgent only when something changed.

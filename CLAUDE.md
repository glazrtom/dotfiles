...
- **`playbooks/k8s.yml`** is the Kubernetes workstation toolkit: `kubectl` role (kubectl + helm + krew + the `kubectl-netshoot` krew plugin, cross-platform incl. a direct-download fallback) + `k9s` role (Homebrew on macOS, pacman on Arch, GitHub release tarball into `~/.local/bin` on Debian/Ubuntu and other Linux since there's no apt/generic package).
